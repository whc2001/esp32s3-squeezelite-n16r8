...
const DefaultStringVal defaultStringVals[] = {
    ...
    {"sleep_config", ""},
    {"led_brightness", ""},
    {"spdif_config", "bck=38,ws=39,do=40"},
    {"dac_config", ""},
    {"dac_controlset", ""},
    {"jack_mutes_amp", "n"},
...
void app_main()
{
	...
	// FIXED: Always run as main app, not recovery mode
	is_recovery_running = false;
	xReason = esp_reset_reason();
	ESP_LOGI(TAG,"Reset reason is: %lu", (unsigned long)xReason);
	if(!is_recovery_running )  {
		...
		ESP_LOGI(TAG,"Reboot counter=%lu\n", (unsigned long)Counter) ;
		if (Counter == 5) {
			guided_factory();
		}
	}
		...
		ESP_LOGI(TAG,"Recovery Reboot counter=%lu\n", (unsigned long)Counter) ;
	...
	/* Initialize console BEFORE network_start() because HTTP server uses console_push() 
	 * for /commands.json handler. Console must be ready before HTTP requests arrive! */
