    ...
    while (1) {
		...
		if (xActivatedMember == button_queue) {
			...
			*/
			...
			ESP_LOGD(TAG, "Rotary event: position %ld, direction %s", (long)event.state.position,
			...
			ESP_LOGD(TAG, "Volume event: position %ld, direction %s", (long)event.state.position,
					event.state.direction ? (event.state.direction == ROTARY_ENCODER_DIRECTION_CLOCKWISE ? "CW" : "CCW") : "NOT_SET");
