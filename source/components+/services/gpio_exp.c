...
gpio_exp_t* gpio_exp_create(const gpio_exp_config_t *config) {
	...
	if (config->base < GPIO_NUM_MAX || n_expanders == sizeof(expanders)/sizeof(gpio_exp_t)) {
		ESP_LOGE(TAG, "Base %lu GPIO must be at least %d for %s or too many expanders %d", (unsigned long)config->base, GPIO_NUM_MAX, config->model, n_expanders);
		return NULL;
	}
	...
	ESP_LOGI(TAG, "Create GPIO expander %s at base %lu with intr %d at @%lx on port/host %d/%d", config->model, (unsigned long)config->base, config->intr, (unsigned long)config->phy.addr, config->phy.port, config->phy.host);
	return expander;
}
...
esp_err_t gpio_exp_set_direction(int gpio, gpio_mode_t mode, gpio_exp_t *expander) {
	...
	if (expander->r_mask & expander->w_mask) {
		...
		ESP_LOGE(TAG, "GPIO %d on expander base %lu can't be r/w", gpio, (unsigned long)expander->first);
...
int gpio_exp_get_level(int gpio, int age, gpio_exp_t *expander) {
	...
	if (xSemaphoreTake(expander->mutex, pdMS_TO_TICKS(50)) == pdFALSE) {
		ESP_LOGW(TAG, "Can't get mutex for GPIO %lu", (unsigned long)(expander->first + gpio));
	...
	ESP_LOGD(TAG, "Get level for GPIO %lu => read %lx", (unsigned long)(expander->first + gpio), (unsigned long)expander->shadow);
	return (expander->shadow >> gpio) & 0x01;
}
...
esp_err_t gpio_exp_set_level(int gpio, int level, bool direct, gpio_exp_t *expander) {
	...
	if ((expander->w_mask & mask) == 0) {
		ESP_LOGW(TAG, "GPIO %d is not set for output", (int)(expander->first + gpio));
		return ESP_ERR_INVALID_ARG;
	}
	...
	if (direct) {
		...
		xSemaphoreGive(expander->mutex);
		ESP_LOGD(TAG, "Set level %x for GPIO %u => wrote %lx", level, (unsigned)(expander->first + gpio), (unsigned long)expander->shadow);
	} else {
	...
	while (1) {
		...
		if (notif == GPIO_EXP_INTR) {
			...
			for (int i = 0; i < n_expanders; i++) {
				...
				xSemaphoreGive(expander->mutex);
				ESP_LOGD(TAG, "Handling GPIO %ld reads 0x%04lx and has 0x%04lx pending", (long)expander->first, (unsigned long)expander->shadow, (unsigned long)pending);
