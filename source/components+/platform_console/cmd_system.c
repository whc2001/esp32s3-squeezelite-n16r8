...
static int get_version(int argc, char **argv)
{
    ...
    "\trevision number:%d\r\n",
		...
		(unsigned long)(flash_size / (1024 * 1024)), " MB", info.revision);
    return 0;
}
...
cJSON * setdevicename_cb(){
	// Try "name" key first (saved by web UI), then fall back to "host_name"
	char * device_name = config_alloc_get_str("name", NULL, NULL);
	if(!device_name) {
		device_name = config_alloc_get_str("host_name", NULL, "Squeezelite");
	}
	cJSON * values = cJSON_CreateObject();
	cJSON_AddStringToObject(values,"name",device_name);
	free(device_name);
	return values;
}
...
static int setdevicename(int argc, char **argv)
{
	...
	nerrors+=setnamevar("name", f, name);  // save the "name" key for web UI
