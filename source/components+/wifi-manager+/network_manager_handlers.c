...
static state_machine_result_t WIFI_LOST_CONNECTION_STATE_entry_handler(state_machine_t* const State_Machine) {
    ...
    nm->last_connected = 0;
    nm->num_disconnect++;
    ESP_LOGW(TAG, " Wifi disconnected. Number of disconnects: %lu, Average time connected: %lu", (unsigned long)nm->num_disconnect, nm->num_disconnect > 0 ? (unsigned long)(nm->total_connected_time / nm->num_disconnect) : 0UL);
    if (nm->retries < WIFI_MANAGER_MAX_RETRY) {
        ...
        if (network_is_interface_connected(nm->eth_netif)) {
            ...
            network_set_timer(nm->STA_duration, "Wifi Polling timeout");
            ESP_LOGD(TAG, " STA search slow polling of %lu", (unsigned long)nm->STA_duration);
        }
    }
