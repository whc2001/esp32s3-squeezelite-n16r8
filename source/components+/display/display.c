...
// Global function pointer (definition for extern declaration in display.h)
	...
	while (1) {
		...
		if (displayer.timer && displayer.state == DISPLAYER_ACTIVE) {
			...
			if (elapsed >= 1000) {
				...
				else sprintf(_line, "%lu:%02lu:%02lu", (unsigned long)((elapsed / 3600) % 100), (unsigned long)((elapsed % 3600) / 60), (unsigned long)(elapsed % 60));
...
if (displayer.duration.value > 3600) sprintf(displayer.duration.string, "%lu:%02lu:%02lu", (unsigned long)((displayer.duration.value / 3600) % 10),
															(unsigned long)((displayer.duration.value % 3600) / 60), (unsigned long)(displayer.duration.value % 60));
		else sprintf(displayer.duration.string, "%lu:%02lu", (unsigned long)(displayer.duration.value / 60), (unsigned long)(displayer.duration.value % 60));
