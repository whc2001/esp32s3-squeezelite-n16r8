...
static void event_logger(uint32_t state_machine, uint32_t state, uint32_t event) {
    ESP_LOGD(TAG, "Handling network manager event state Id %lu->[%s]", (unsigned long)state, event_to_string(event));
}
...
static void result_logger(uint32_t state, state_machine_result_t result) {
    ESP_LOGD(TAG, "Network Manager Result: %s, New State id: %lu", get_state_machine_result_string(result) , (unsigned long)state);
}
