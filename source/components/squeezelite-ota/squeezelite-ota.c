...
esp_err_t _erase_last_boot_app_partition(const esp_partition_t *ota_partition)
{
	...
	if(ota_erase_size!=NULL) {
		...
		ESP_LOGD(TAG,"OTA Erase block size is %lu (from string: %s)",(unsigned long)single_pass_size, ota_erase_size );
		free(ota_erase_size);
	}
	else {
	...
	if(single_pass_size % SPI_FLASH_SEC_SIZE !=0){
		...
		ESP_LOGW(TAG,"Invalid erase block size of %lu. Value should be a multiple of %d and will be adjusted to %lu.", (unsigned long)single_pass_size, SPI_FLASH_SEC_SIZE,(unsigned long)temp_single_pass_size);
		single_pass_size=temp_single_pass_size;
	}
	ESP_LOGD(TAG,"Erasing flash partition of size %lu in blocks of %lu bytes", (unsigned long)ota_partition->size, (unsigned long)single_pass_size);
	num_passes=ota_partition->size/single_pass_size;
	remain_size=ota_partition->size-(num_passes*single_pass_size);
	ESP_LOGI(TAG,"Erasing in %lu passes with blocks of %lu bytes ", (unsigned long)num_passes,(unsigned long)single_pass_size);
	for(uint16_t i=0;i<num_passes;i++){
		ESP_LOGD(TAG,"Erasing flash (%u%%)",i/num_passes);
		ESP_LOGD(TAG,"Pass %d of %lu, with chunks of %lu bytes, from %lu to %lu", i+1, (unsigned long)num_passes,(unsigned long)single_pass_size,(unsigned long)(i*single_pass_size),(unsigned long)(i*single_pass_size+single_pass_size));
...
esp_err_t ota_header_check(){
    ...
    ESP_LOGD(TAG, "Running partition [%s] type %d subtype %d (offset 0x%08lx)", ota_status->running->label, ota_status->running->type, ota_status->running->subtype, (unsigned long)ota_status->running->address);
    ...
    if (ota_status->configured != ota_status->running) {
        ESP_LOGW(TAG, "Configured OTA boot partition at offset 0x%08lx, but running from offset 0x%08lx", (unsigned long)ota_status->configured->address, (unsigned long)ota_status->running->address);
        ESP_LOGW(TAG, "(This can happen if either the OTA boot data or preferred boot image become corrupted somehow.)");
    }
    ESP_LOGD(TAG, "Next ota update partition is: [%s] subtype %d at offset 0x%lx",
    		ota_status->update_partition->label, ota_status->update_partition->subtype, (unsigned long)ota_status->update_partition->address);
...
void ota_task(void *pvParameter)
{
    ...
    while (ota_status->remain_image_len>0) {
        ...
        if (data_read <= 0) {
            ...
            ESP_LOGD(TAG, "Written image length %lu", (unsigned long)ota_status->actual_image_len);
			...
			if(ota_status->lastpct!=ota_status->newpct ) {
				...
				gettimeofday(&tv, NULL);
				uint32_t elapsed_ms= (tv.tv_sec-ota_status->OTA_start.tv_sec )*1000+(tv.tv_usec-ota_status->OTA_start.tv_usec)/1000;
				ESP_LOGI(TAG,"OTA progress : %lu/%.0f (%d pct), %lu KB/s", (unsigned long)ota_status->actual_image_len, ota_status->total_image_len, ota_status->newpct, elapsed_ms>0?(unsigned long)(ota_status->actual_image_len*1000/elapsed_ms/1024):0UL);
    ...
    ESP_LOGI(TAG, "Total Write binary data length: %lu", (unsigned long)ota_status->actual_image_len);
...
esp_err_t process_recovery_ota(const char * bin_url, char * bin_buffer, uint32_t length){
	...
	if(num_buffer!=NULL) {
  	...
  	ESP_LOGD(TAG,"OTA task stack size %lu, priority %d (%d %s ESP_TASK_MAIN_PRIO)",(unsigned long)stack_size , task_priority, abs(task_priority-ESP_TASK_MAIN_PRIO), task_priority-ESP_TASK_MAIN_PRIO>0?"above":"below");
