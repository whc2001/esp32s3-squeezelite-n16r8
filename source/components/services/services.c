...
static void sleep_timer(uint32_t now) {
    ...
    if (sleep_context.spurious && now > sleep_context.spurious + first) {
        ...
        if (spurious) {
            ESP_LOGI(TAG, "spurious wake of %lu sec, going back to sleep", (unsigned long)((now - first) / 1000));
        ...
        ESP_LOGI(TAG, "wake-up was not spurious after %lu sec", (unsigned long)((now - first) / 1000));
    }
...
void services_sleep_init(void) {
    ...
    if (cause == ESP_SLEEP_WAKEUP_EXT0 || cause == ESP_SLEEP_WAKEUP_EXT1) {
        ...
        if (infrared_gpio() >= 0 && ((1LL << infrared_gpio()) & wake_gpio)) {
            ...
            ESP_LOGI(TAG, "spurious wake-up detection during %lu sec", (unsigned long)(sleep_context.spurious / 1000));
        }
    }
    ...
    if (sleep_context.delay || sleep_context.spurious) {
        ...
        if (sleep_context.delay) ESP_LOGI(TAG, "inactivity timer of %lu minute(s)", (unsigned long)(sleep_context.delay / (60*1000)));
    }
}
...
void services_init(void) {
	...
	ESP_LOGI(TAG,"Configuring I2C sda:%d scl:%d port:%u speed:%lu", i2c_config->sda_io_num, i2c_config->scl_io_num, i2c_system_port, (unsigned long)i2c_config->master.clk_speed);
