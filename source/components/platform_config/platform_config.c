...
cJSON * existing = cJSON_GetObjectItemCaseSensitive(nvs_json, key);
	...
	if(existing!=NULL ) {
		...
		if(!cJSON_Compare(entry,existing,false)){
			...
			// Only mark as changed if not loading from NVS
		...
		// Only mark as changed if not loading from NVS
		bool bIsConfigLoading = ((xEventGroupGetBits(config_group) & CONFIG_LOAD_BIT) != 0);
		config_set_entry_changed_flag(entry, !bIsConfigLoading);
...
void config_set_entry_changed_flag(cJSON * entry, cJSON_bool flag){
	ESP_LOGD(TAG, "config_set_entry_changed_flag: flag=%d", flag);
	if(entry==NULL){
		ESP_LOGE(TAG,"null pointer received!");
		return;
	}
	// Always use the flag as passed - caller decides if change should be marked
	bool changedFlag = flag;
	cJSON * changed = cJSON_GetObjectItemCaseSensitive(entry, "chg");
	if(changed ==NULL ) {
		ESP_LOGI(TAG, "config_set_entry_changed_flag: chg flag not found. Adding changedFlag=%d", changedFlag);
		cJSON_AddBoolToObject(entry,"chg",changedFlag);
	}
	else {
		ESP_LOGI(TAG, "config_set_entry_changed_flag: Existing chg=%d, setting to changedFlag=%d", cJSON_IsTrue(changed), changedFlag);
		// Always update the flag - use cJSON_ReplaceItemInObject for reliability
		cJSON_DeleteItemFromObject(entry, "chg");
		cJSON_AddBoolToObject(entry, "chg", changedFlag);
		ESP_LOGI(TAG, "config_set_entry_changed_flag: Updated change flag to %s", changedFlag?"TRUE":"FALSE");
	}
...
void config_commit_to_nvs(){
	...
	ESP_LOGI(TAG,"config_commit_to_nvs. Config Locked!");
	cJSON * entry=nvs_json->child;
	while(entry!= NULL){
		char * entry_str = cJSON_PrintUnformatted(entry);
		if(entry_str!=NULL){
			ESP_LOGI(TAG,"config_commit_to_nvs processing item %s",entry_str);
			free(entry_str);
		}

		if(config_is_entry_changed(entry)){
			ESP_LOGI(TAG, "Committing entry %s value to nvs.",(entry->string==NULL)?"UNKNOWN":entry->string);
			nvs_type_t type = config_get_entry_type(entry);
			...
			ESP_LOGI(TAG,"config_commit_to_nvs. Item [%s] already committed (chg=false). Ignoring.", entry->string ? entry->string : "NULL");
		}
...
esp_err_t config_set_value(nvs_type_t nvs_type, const char *key, const void * value){
	...
	bool bIsConfigLoading = ((xEventGroupGetBits(config_group) & CONFIG_LOAD_BIT) != 0);
	ESP_LOGI(TAG, "config_set_value called for key [%s], CONFIG_LOAD_BIT=%d", key ? key : "NULL", bIsConfigLoading);
	if(!key ||!key[0]){
...
// Force set value and mark as changed - for HTTP POST saves
esp_err_t config_set_value_forced(nvs_type_t nvs_type, const char *key, const void * value){
	esp_err_t result = ESP_OK;
	ESP_LOGI(TAG, "config_set_value_forced called for key [%s]", key ? key : "NULL");
	if(!key ||!key[0]){
		ESP_LOGW(TAG,"Empty key passed. Ignoring entry!");
		return ESP_ERR_INVALID_ARG;
	}
	...
	if(entry == NULL){
		...
		// Force mark as changed regardless of comparison result
		config_set_entry_changed_flag(entry, true);
		ESP_LOGI(TAG,"config_set_value_forced: marked [%s] as changed", key);
	}
	config_unlock();
	return result;
}
