/*  ----------------------------------------------------------------------------
    ...
    Updated: Reverted to Legacy RMT API for IDF 5.x compatibility with led.c/infrared.c
...
// Store RMT channel for each strip
static rmt_channel_t rmt_channel_used = RMT_CHANNEL_MAX;
...
/****************************************************************************************
 * APA102 support using GPIO (not RMT)
 ****************************************************************************************/
...
/****************************************************************************************
 * LED strip task - handles both RMT and APA102 LED types
 ****************************************************************************************/
static void led_strip_task(void *arg)
{
    ...
    if (led_strip->rgb_led_type == RGB_LED_TYPE_APA102) {
        ...
        // RMT-based LED types (WS2812, SK6812, APA106)
        ...
        for(;;) {
            rmt_wait_tx_done(rmt_channel_used, portMAX_DELAY);
            vTaskDelay(LED_STRIP_REFRESH_PERIOD_MS / portTICK_PERIOD_MS);
            ...
            rmt_write_items(rmt_channel_used,
                            rmt_items,
                            num_items_malloc,
                            false);
        }
...
{
    // Get next available RMT TX channel
    rmt_channel_used = RMT_NEXT_TX_CHANNEL();
    
    rmt_config_t rmt_cfg = {
        .rmt_mode = RMT_MODE_TX,
        .channel = rmt_channel_used,
    ...
    if (cfg_ok != ESP_OK) {
        ESP_LOGE(TAG, "RMT config failed: %s", esp_err_to_name(cfg_ok));
        return false;
    }
    esp_err_t install_ok = rmt_driver_install(rmt_cfg.channel, 0, 0);
    if (install_ok != ESP_OK) {
        ESP_LOGE(TAG, "RMT driver install failed: %s", esp_err_to_name(install_ok));
        return false;
    }

    ESP_LOGI(TAG, "RMT LED strip initialized on GPIO %d, channel %d", led_strip->gpio, rmt_channel_used);
    return true;
}
