...
Mat::~Mat()
{
    ESP_LOGD("Mat", "~Mat(%i, %i), ext_buff=%i, data=0x%8.8" PRIx32, this->rows, this->cols, this->ext_buff, (uint32_t)this->data);
...
void Mat::allocate()
{
    ...
    ESP_LOGD("Mat", "allocate(%i) = 0x%8.8" PRIx32, this->length, (uint32_t)this->data);
}
